    ...
    <div class="w-full max-w-xl px-6 relative z-10">
        ...
        <div class="bg-zinc-950/80 backdrop-blur-xl border border-white/10 rounded-xl p-8 md:p-10">
            <div class="mb-8 border-b border-white/10 pb-6">
                <h2 class="text-2xl font-semibold text-white">Añadir Ejemplar</h2>
                <p class="text-sm text-zinc-500 mt-1">Introduce los metadatos del nuevo libro.</p>
            </div>
            ...
            <form action="{{ route('books.store') }}" method="POST" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="titulo" class="input-dark" required autocomplete="off">
                </div>

                <div>
                    <label class="label-dark">Autor</label>
                    <input type="text" name="autor" class="input-dark" required autocomplete="off">
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        <label class="label-dark">Año de Publicación</label>
                        <input type="number" name="anio_publicacion" class="input-dark" required>
                    </div>
                    <div>
                        <label class="label-dark">Categoría</label>
                        <input type="text" name="categoria" class="input-dark" required autocomplete="off">
                    </div>
                </div>

                <div class="pt-4">
                    <label class="flex items-center cursor-pointer">
                        <input type="checkbox" name="disponible" value="1" checked class="w-4 h-4 rounded border-gray-800 bg-zinc-900 text-blue-500 focus:ring-blue-500/50 focus:ring-offset-0">
                ...
                <button type="submit" class="w-full mt-6 px-4 py-3 bg-white text-black font-medium text-sm hover:bg-zinc-200 transition-colors rounded-md">
                    Guardar Registro
                </button>
            </form>
        </div>
