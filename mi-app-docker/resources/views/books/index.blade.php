    ...
    <div class="container mx-auto px-6 max-w-6xl relative z-10">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 border-b border-white/10 pb-6">
            ...
            <div class="mt-6 md:mt-0 flex gap-3">
                <a href="{{ route('borrow.inventory', ['user_id' => 1]) }}" class="inline-block px-5 py-2.5 border border-white/10 text-zinc-400 font-mono text-sm hover:text-white hover:bg-white/5 transition-colors rounded-md">
                    > Mi_Inventario
                </a>
        ...
        @if(session('success'))
            <div class="mb-8 p-4 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-mono rounded-md">
                [SISTEMA]: {{ session('success') }}
            </div>
        @endif
            ...
            <table class="w-full text-left border-collapse min-w-[800px]">
                <thead>
                    <tr class="font-mono text-xs uppercase tracking-wider text-zinc-500 border-b border-white/10 bg-zinc-900/50">
                        ...
                        <th class="p-4 font-normal">Disponibilidad</th>
                    ...
                    <tr class="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                        <td class="p-4 flex items-center gap-4">
                            <div class="relative h-16 w-12 rounded-md border border-white/10 bg-zinc-900/50 overflow-hidden shadow-inner flex items-center justify-center flex-shrink-0">
                                ...
                                <span class="cover-fallback-list text-zinc-700 absolute inset-0 flex items-center justify-center p-2 z-0">
                                    <svg class="w-5 h-5 opacity-20" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg>
                                </span>
                            </div>
                            <div>
                        ...
                        <td class="p-4">
                            ...
                            @else
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-xs text-zinc-500">Sin stock</span>
                                </div>
                            @endif
                        </td>
                        <td class="p-4 text-right">
                            <div class="flex justify-end gap-3 text-xs font-medium items-center">
                                @if($book->stock > 0)
                                <form action="{{ route('borrow.store', ['user_id' => 1, 'book_id' => $book->id]) }}" method="POST" class="inline m-0">
                                    @csrf
                                    <button type="submit" class="text-emerald-500 hover:text-emerald-300 transition-colors uppercase text-[10px] font-mono tracking-tighter">Reservar</button>
                                </form>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Carga inmediata en paralelo
        covers.forEach(img => cargarPortada(img));

        async function cargarPortada(img) {
            ...
            try {
                // INTENTO 1: Open Library (Más rápido para portadas de libros clásicos)
                const resOL = await fetch(`https://openlibrary.org/search.json?title=${encodeURIComponent(titulo)}&limit=1`);
                const dataOL = await resOL.json();
                
                if (dataOL.docs && dataOL.docs.length > 0 && dataOL.docs[0].cover_i) {
                    mostrarImagen(`https://covers.openlibrary.org/b/id/${dataOL.docs[0].cover_i}-M.jpg`);
                    return;
                }

                // INTENTO 2: Google Books (Plan B, más exhaustivo)
                const resGB = await fetch(`https://www.googleapis.com/books/v1/volumes?q=intitle:${encodeURIComponent(titulo)}+inauthor:${encodeURIComponent(autorRaw)}&maxResults=1`);
                const dataGB = await resGB.json();
                
                if (dataGB.items && dataGB.items[0].volumeInfo.imageLinks) {
                    let url = dataGB.items[0].volumeInfo.imageLinks.thumbnail.replace('http:', 'https:');
                    mostrarImagen(url);
                }
            } catch (error) {
                console.error("Fallo de conexión para: " + titulo);
            }
        }
    });
</script>
